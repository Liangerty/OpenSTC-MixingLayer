...
__global__ void compute_convective_term_ep_1D(DZone *zone, int direction, int max_extent, DParameter *param) {
  ...
  if (tid > block_dim - ngg - 1 || idx[direction] > max_extent - ngg - 1) {
    const int igShared = tid + 2 * ngg - 1;
    const int rGhIdx[3]{idx[0] + ngg * labels[0], idx[1] + ngg * labels[1], idx[2] + ngg * labels[2]};
  ...
  if (idx[direction] == max_extent - 1 && tid < ngg - 1) {
    ...
    for (int m = 0; m < n_more_right; ++m) {
      const int igShared = i_shared + m + 1;
      const int rGhIdx[3]{idx[0] + (m + 1) * labels[0], idx[1] + (m + 1) * labels[1], idx[2] + (m + 1) * labels[2]};
      metric[igShared * 3] = zone->metric(rGhIdx[0], rGhIdx[1], rGhIdx[2])(direction + 1, 1);
      metric[igShared * 3 + 1] = zone->metric(rGhIdx[0], rGhIdx[1], rGhIdx[2])(direction + 1, 2);
      metric[igShared * 3 + 2] = zone->metric(rGhIdx[0], rGhIdx[1], rGhIdx[2])(direction + 1, 3);
      uk_jac[igShared] = metric[igShared * 3] * pv(rGhIdx[0], rGhIdx[1], rGhIdx[2], 1)
...
template void
compute_convective_term_pv<MixtureModel::Air>(const Block &block, DZone *zone, DParameter *param, int n_var,
                                              const Parameter &parameter);

template void
compute_convective_term_pv<MixtureModel::Mixture>(const Block &block, DZone *zone, DParameter *param,
                                                  int n_var, const Parameter &parameter);

template void
compute_convective_term_pv<MixtureModel::MixtureFraction>(const Block &block, DZone *zone, DParameter *param,
                                                          int n_var, const Parameter &parameter);

template void
compute_convective_term_pv<MixtureModel::FR>(const Block &block, DZone *zone, DParameter *param, int n_var,
                                             const Parameter &parameter);

template void
compute_convective_term_pv<MixtureModel::FL>(const Block &block, DZone *zone, DParameter *param, int n_var,
                                             const Parameter &parameter);

template void
compute_convective_term_aweno<MixtureModel::Air>(const Block &block, DZone *zone, DParameter *param, int n_var);

template void
compute_convective_term_aweno<MixtureModel::Mixture>(const Block &block, DZone *zone, DParameter *param,
                                                     int n_var);

template void
compute_convective_term_aweno<MixtureModel::MixtureFraction>(const Block &block, DZone *zone, DParameter *param,
                                                             int n_var);

template void
compute_convective_term_aweno<MixtureModel::FR>(const Block &block, DZone *zone, DParameter *param, int n_var);

template void
compute_convective_term_aweno<MixtureModel::FL>(const Block &block, DZone *zone, DParameter *param, int n_var);

template
void Roe_compute_inviscid_flux<MixtureModel::Air>(const Block &block, DZone *zone, DParameter *param,
                                                  int n_var, const Parameter &parameter);

template
void Roe_compute_inviscid_flux<MixtureModel::Mixture>(const Block &block, DZone *zone, DParameter *param,
                                                      int n_var, const Parameter &parameter);

template
void Roe_compute_inviscid_flux<MixtureModel::FR>(const Block &block, DZone *zone, DParameter *param,
                                                 int n_var, const Parameter &parameter);

template<>
void Roe_compute_inviscid_flux<MixtureModel::FL>(const Block &block, DZone *zone, DParameter *param,
                                                 int n_var, const Parameter &parameter) {
...
template<>
void Roe_compute_inviscid_flux<MixtureModel::MixtureFraction>(const Block &block, DZone *zone, DParameter *param,
                                                              int n_var, const Parameter &parameter) {
...
template void
compute_convective_term_ep<MixtureModel::Air>(const Block &block, DZone *zone, DParameter *param, int n_var);

template void
compute_convective_term_ep<MixtureModel::Mixture>(const Block &block, DZone *zone, DParameter *param, int n_var);

template<>
void
compute_convective_term_ep<MixtureModel::MixtureFraction>(const Block &block, DZone *zone, DParameter *param,
                                                          int n_var) {
...
template void
compute_convective_term_ep<MixtureModel::FR>(const Block &block, DZone *zone, DParameter *param, int n_var);

template<>
void
compute_convective_term_ep<MixtureModel::FL>(const Block &block, DZone *zone, DParameter *param, int n_var) {
