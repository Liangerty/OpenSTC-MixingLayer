...
template<MixtureModel mix_model, class turb, bool with_cv = false>
void parallel_communication(const Mesh &mesh, std::vector<cfd::Field> &field, integer step, const Parameter &parameter,
                            DParameter *param);
...
template<MixtureModel mix_model, class turb, bool with_cv>
void parallel_communication(const Mesh &mesh, std::vector<Field> &field, integer step, const Parameter &parameter,
                            DParameter *param) {
  const int n_block{mesh.n_block};
  const integer n_trans{parameter.get_int("n_scalar") + 6}; // All primitive variables are to be transferred
